		...
		int type, dev_t rdev, struct svc_fh *resfhp)
{
	...
	switch (type) {
	case S_IFREG:
		host_err = vfs_create(dirp, dchild, iap->ia_mode, true, NULL);
		if (!host_err)
				...
				char *name, int len, struct svc_fh *tfhp)
{
	...
	if (!dold->d_inode)
		goto out_dput;
	host_err = vfs_link(dold, dirp, dnew, NULL, NULL);
	if (!host_err) {
			    ...
			    struct svc_fh *tfhp, char *tname, int tlen)
{
	...
	host_err = vfs_rename(fdir, odentry, tdir, ndentry, NULL, 0, NULL);
	if (!host_err) {
				...
				char *fname, int flen)
{
	...
	if (type != S_IFDIR)
		host_err = vfs_unlink(dirp, rdentry, NULL, NULL);
	else
