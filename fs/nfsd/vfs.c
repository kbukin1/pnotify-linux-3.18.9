		...
		int type, dev_t rdev, struct svc_fh *resfhp)
{
	...
	switch (type) {
	case S_IFREG:
		host_err = vfs_create(dirp, dchild, iap->ia_mode, true, 0);
		if (!host_err)
			nfsd_check_ignore_resizing(iap);
		break;
	case S_IFDIR:
		host_err = vfs_mkdir(dirp, dchild, iap->ia_mode, 0);
		break;
	case S_IFCHR:
	case S_IFBLK:
	case S_IFIFO:
	case S_IFSOCK:
		host_err = vfs_mknod(dirp, dchild, iap->ia_mode, rdev, 0);
		break;
	}
	if (host_err < 0)
		goto out_nfserr;
				...
				struct svc_fh *resfhp)
{
	...
	host_err = vfs_symlink(dentry->d_inode, dnew, path, 0);
	err = nfserrno(host_err);
	if (!err)
		err = nfserrno(commit_metadata(fhp));
	fh_unlock(fhp);
			    ...
			    struct svc_fh *tfhp, char *tname, int tlen)
{
	...
	host_err = vfs_rename(fdir, odentry, tdir, ndentry, NULL, 0, 0);
	if (!host_err) {
				...
				char *fname, int flen)
{
	...
	if (type != S_IFDIR)
		host_err = vfs_unlink(dirp, rdentry, NULL, 0);
	else
